/* gcc.c -- changed for emx by Eberhard Mattes -- Jan 1992 */
...
char *standard_exec_prefix_1 = "gcc-";
...
/* Fork one piped subcommand.  FUNC is the system call to use
   (either spawnv or spawnvp) [deleted].  ARGV is the arg vector to use.
     ...
     int not_last;
{
  ...
  if (not_last)
    {
    fprintf (stderr, "gcc: pipes not supported\n");
    exit (250);
    }
  else
    last_pipe_input = STDIN_FILE_NO;

  rc = spawnvp (P_WAIT, program, (char const * const *)argv);
  if (rc < 0)
      pfatal_with_name (program);
global_rc = rc;
}

