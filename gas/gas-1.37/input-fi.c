/* input-fi.c -- changed for emx by Eberhard Mattes -- Feb 1992 */
     ...
     int	debugging;	/* TRUE if we are debugging assembler. */
{
  ...
  if (filename [0])
    {				/* We have a file name. Suck it and see. */
      file_handle = open (filename, O_RDONLY | O_TEXT, 0);
      file_name = filename;
    }
  else
 ...
 */
      ...
      if (pid == 0)
	{
	  (void)dup2 (file_handle, fileno(stdin));
          fd = open (temporary_file_name, O_WRONLY + O_TRUNC + O_CREAT + O_TEXT, 0666);
	  if (fd == -1)
	    {
...
#define PREPROCESSOR "./app"
	...
	{
          file_handle = open (temporary_file_name, O_RDONLY | O_TEXT, 0);
	  if ( ! debugging )
	    {
     ...
     int pre;
{
	...
	if ( preprocess ) {
		...
		(void)mktemp (temporary_file_name);
                f_out=fopen(temporary_file_name,"wt+");
		if(f_out==(FILE *)0) {
