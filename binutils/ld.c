/* ld.c -- changed for emx by Eberhard Mattes -- Feb 1992 */
     ...
     register struct file_entry *entry;
{
  ...
  if (entry->file_type == IS_ARCHIVE)
    ...
    {
      read_entry_symbols (desc, entry);
      entry->strings = xmalloc (entry->strs_size);
      read_entry_strings (desc, entry);
     ...
     struct file_entry **prev_addr, *entry;
{
  ...
  if (!subfile_wanted_p (subentry))
    {
      if (subentry->strings) free (subentry->strings);
      free (subentry->symbols);
      free (subentry);
    }
  else
     ...
     FILE *outfile;
{
  ...
  unsigned char *nlist_bitvector
    = (unsigned char *) xmalloc ((number_of_syms >> 3) + 1);
...
void check_exe (void)
    ...
    for (p = output_filename; *p != 0; ++p)
        if (*p == ':' || *p == '\\' || *p == '/')
            name = p + 1;
    p = strchr (name, '.');
    if (p == NULL || stricmp (p + 1, "exe") != 0)
        {
        exe_filename = NULL;
        return;
        }
    ...
    (void)memcpy (output_filename, exe_filename, i);
...
write_output ()
{
  ...
  if (exe_filename != NULL)
    ...
    if (_path (emxl_path, "emxl.exe") == 0)
        ...
        nargv[2] = "/emx/bin/emxl.exe";
    nargv[3] = output_filename;
    nargv[4] = exe_filename;
    nargv[5] = NULL;
     ...
     struct file_entry *entry;
{
  ...
  register int desc;
  register struct relocation_info *reloc;
  struct relocation_info *reloc1 = 0;
  ...
  if (entry->textrel)  reloc = entry->textrel;
    ...
    {
      reloc = reloc1 = (struct relocation_info *) xmalloc (entry->text_reloc_size);
      lseek (desc, entry->starting_offset + entry->text_reloc_offset, L_SET);
     ...
     struct file_entry *entry;
{
  ...
  if (entry->datarel) reloc = entry->datarel;
    ...
    {
      reloc = reloc1 = (struct relocation_info *) xmalloc (entry->data_reloc_size);
      lseek (desc, entry->starting_offset + entry->data_reloc_offset, L_SET);
      if (entry->data_reloc_size != read (desc, reloc, entry->data_reloc_size))
	fatal_with_file ("premature eof in data relocation of ", entry);
#ifdef MACH_O
...
write_syms ()
{
  ...
  write_string_table ();
  free (buf); free (strtab_vector); free (strtab_lens);
}

     ...
     int *syms_written_addr;
{
  ...
  struct nlist *buf
    = (struct nlist *) xmalloc (entry->syms_size + sizeof (struct nlist));
  register struct nlist *bufp = buf;
