...
#define DEFAULT_INFOPATH ".;/emx/info;/usr/gnu/info;/usr/local/emacs/info;/usr/local/lib/emacs/info"
...
/* If non-NULL, this is a semi-colon separated list of directories to search
     ...
     char **argv;
{
  ...
  if (!get_node (filename, (nodenames[0] == NULL) ? "" : nodenames[0], 0)
      && !get_node ((char *)NULL, (char *)NULL, 1))
      {
	if (!*filename)
	  strcpy (filename, "dir");

	fprintf (stderr, "%s: Cannot find \"%s\", anywhere along the search ",
		 argv[0], filename);
...
void info_signal_handler (int sig);
...
begin_info_session ()
{
  ...
  print_string ("Welcome to Info!  Press <F1> for help. ");
  close_echo_area ();
  toploop ();
...
void add_completion (char *identifier, char *data, int position)
{
     ...
     int remember_name;
{
  ...
  if (result || (input_stream = fopen (tempname, "rt")) == NULL)
    {
      file_error (tempname);
      return (0);
    }
  ...
  info_buffer_len = fread (info_file, 1, info_buffer_len, input_stream);
  fclose (input_stream);
     ...
     char *filename;
{
  int i = nodetop;
  int c;
  FILE *output_stream = fopen (filename, "wt");
...
void bash_window_to(int position)
{
	int pointer;

	pointer = back_lines ((the_window.bottom - the_window.top) / 2
		, forward_lines (1, position));
...
toploop ()
{
	int done, inhibit_display, new_node, menu_item;
	...
	menu_item = new_node = new_ypos = last_pagetop = -1;
	menuname[0] = '\0';

	while (!done)
   	{
    	if (!inhibit_display && (window_bashed || (pagetop != last_pagetop)))
			display_page ();
		...
		if (menu_item < 0 || new_ypos < pagetop || new_ypos >= pagebot)
		{
	  		COMP_ENTRY *mp = completion_list;

			new_ypos = -1;
			menu_item = 0;
			menuname[0] = '\0';
			for (menu_item = 0; mp; menu_item++, mp = mp->next)
				if (mp->position >= pagetop && mp->position < pagebot)
				{
					new_ypos = mp->position;
					strcpy(menuname, mp->identifier);
					break;
				}
			if (new_ypos == -1)
				menu_item = -1;
		...
		if (new_ypos >= 0)
		{
			if (cursor_to(new_ypos) && menu_item >= 0)
			{
				if (terminal_inverse_begin)
					do_term (terminal_inverse_begin);
				print_string ("%s", menuname);
				if (terminal_inverse_begin)
		...
		case KBackTab:
			inhibit_display = 1;
			if (menu_item >= 0 && cursor_to(new_ypos))
			{
				print_string ("%s", menuname);
				cursor_to(new_ypos);
			}
		...
		case KTab:
			...
			if (the_menu_size)
			{
				...
				if (++menu_item >= the_menu_size)
					menu_item = the_menu_size - 1;
				for (i = 0; mp; i++, mp = mp->next)
					if (i == menu_item)
					{
						new_ypos = mp->position;
		...
		case KEnter:
		{
			...
			for (; mp; mp = mp->next)
				if (mp->position == new_ypos)
				{
					if (get_node ((char *) NULL, mp->data, 0))
					{
	    				clear_echo_area ();
						new_node = 1;
					}
			...
			{
				inhibit_display =1;
	    		display_error ("No menu items are available!");
			}
			break;
		}
		...
		case 'H':
			if ((the_window.bottom - the_window.top) < 24)
	    		get_node ((char *) NULL, "(info.info)Help-Small-Screen", 0);
			else
	    		get_node ((char *) NULL, "(info.info)Help", 0);
			new_node = 1;
			break;
		...
		case 'S':
		{
...

	    	{
				...
				{
					display_error("%d: There doesn't appear to be a nodename for this node.", pointer);
	...
	/*	case CTRL ('H'):	*/
		case KF1:
		case '?':
    		get_node ((char *) NULL, "(dir)HelpKeys", 0);
			break;
...
build_menu ()
{
  ...
  while (1)
      ...
      else
	{
      ...
      add_completion (menuname, nodename, menu_pointer);
      the_menu_size++;
    }
  if (the_menu_size)
...
build_notes ()
{
  ...
  while (1)
      ...
      else
	{
      ...
      add_completion (notename, nodename, menu_pointer);
      the_menu_size++;
...
/* Move to the end of this node.  Return 0 if we are already there */
int end_page ()
{
  int pointer;

  pointer = back_lines (the_window.bottom - the_window.top - 2, nodebot);
  if (pointer < pagetop)
	return (0);
  pagetop = pointer;
  return (1);
...
until_we_like_it:
  ...
  if (character == ABORT_CHAR || character == KEsc)
    {
      ding ();
      return (0);
    }
...
#ifndef DONT_DO_THIS_THE_EMX_WAY
	character = _read_kbd(0, 1, 0);
	if (character == 0)
		character = -_read_kbd(0, 1, 0);
#else
...
opsys_init_terminal ()
{
  ...
  retval = ioctl (tty, TCSETA, &ttybuff);
