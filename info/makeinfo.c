...
/* Under DOS or OS/2, we can't use stat so we must count the file size
   to know when things should be split.
*/
...
int
  ...
  cm_enumerate (), cm_table (), cm_itemx (), cm_noindent (),
  cm_setfilename (), cm_comment (), cm_br (), cm_sp (), cm_page (),
  cm_group (), cm_need (), cm_center (), cm_include (), cm_bye (),
  cm_item (), cm_end (), cm_infoinclude (), cm_ifinfo (), cm_kindex (),
  cm_cindex (), cm_findex (), cm_pindex (), cm_vindex (),
     ...
     char *filename;
{
  ...
  if (n == -1)
	...
	got = read (file, result, fileinfo.st_size);
     ...
     char *name;
{
  	...
  	/* Try to load the file specified by NAME.  If the file isn't found, and
     	there is no suffix in NAME, then try NAME.texinfo, and NAME.texi, and
	 	NAME.tex. */
...
flush_output ()
{
  ...
  current_file_size += fwrite (output_paragraph, 1, output_paragraph_offset, output_stream);
	...
	for (; tags; tags = tags->next_ent)
	  {
	      ...
	      write_region:
		{
		  ...
		  sprintf (split_file, "%s%s", root_pathname, root_filename);
		  if (!(stp = strrchr(split_file, '.')))
		  	stp = split_file + strlen(split_file);
		  sprintf (stp, ".i%02d", which_file);
		  ...
		  sprintf (split_file, "%s", root_filename);
		  if (!(stp = strrchr(split_file, '.')))
		  	stp = split_file + strlen(split_file);
		  sprintf (stp, ".i%02d", which_file);
  ...
  if (length < fill_column)
    {
      ...
      strncpy (line, (char *)output_paragraph + start, length);
      line[length] = '\0';
