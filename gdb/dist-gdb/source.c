/* source.c -- changed for emx by Eberhard Mattes -- Feb 1992 */
     ...
     int from_tty;
{
  ...
  if (dirname == 0)
      ...
      do
	{
	  ...
	  {
	    char *semicolon = index (name, ';');
	    char *space = index (name, ' ');
	    char *tab = index (name, '\t');
	    if (semicolon == 0 && space == 0 && tab ==  0)
	      p = dirname = name + strlen (name);
	    else
	      {
		p = 0;
		if (semicolon != 0 && (p == 0 || semicolon < p))
		  p = semicolon;
	  ...
	  *p = '\0';
	  ...
	  if (!ABSPATH (name))
	    name = fconcat (current_directory, name);
	  else
...

/* Open a file named STRING, searching path PATH (dir names sep by semicolons)
   using mode MODE and protection bits PROT in the calls to open.
   If TRY_CWD_FIRST, try to open ./STRING before searching PATH.
   (ie pretend the first element of PATH is ".")
   If FILENAMED_OPENED is non-null, set it to a newly allocated string naming
   the actual file opened (this string will always start with a "/" or a
   drive name followed by "/")
     ...
     char **filename_opened;
{
  ...
  /* ./foo => foo */
  while (string[0] == '.' && PATHSEP (string[1]))
    string += 2;

  if (try_cwd_first || ABSPATH (string))
    {
      filename = string;
      fd = open (filename, mode, prot);
	  if (fd >= 0 || ABSPATH (string))
	goto done;
    }
  ...
  if (filename_opened)
    if (fd < 0)
      ...
      *filename_opened = savestring (filename, strlen (filename));
      ...
      {
	*filename_opened = fconcat (current_directory, filename);
      }
     ...
     int desc;
{
  ...
  if ((nread=myread (desc, data, st.st_size)) < 0)
    perror_with_name (s->filename);
  end = data + nread;
  p = data;
  line_charpos[0] = 0;
     ...
     char **fullname;
{
  ...
  desc = openp (source_path, 0, s->filename, O_RDONLY | O_BINARY, 0, &s->fullname);
  if (desc < 0)
    {
      if (fullname)
	*fullname = NULL;
     ...
     char *regex;
{
  ...
  desc = openp (source_path, 0, current_source_symtab->filename,
        O_RDONLY | O_BINARY, 0, &current_source_symtab->fullname);
  if (desc < 0)
