/* main.c -- changed for emx by Eberhard Mattes -- Jan 1992 */
     ...
     int from_tty;
{
  ...
  len = strlen (dir);
  dir = savestring (dir, len - (len > 1 && PATHSEP (dir[len-1])));
  if (ABSPATH (dir))
    current_directory = dir;
  else
    {
      current_directory = fconcat (current_directory, dir);
      free (dir);
    }
  ...
  while (change)
      ...
      for (p = current_directory; *p;)
	{
	  if (!strncmp (p, "/./", 2)
	    ...
	    {
	      char *q = p;
	      while (q != current_directory && !PATHSEP (q[-1])) q--;
...
initialize_main ()
{
  ...
  if (tmpenv = getenv ("GDBHISTFILE"))
    ...
    /* We include the current directory so that if the user changes
       ...
       that was read.  */
    history_filename = fconcat (current_directory, "!gdb_history");
