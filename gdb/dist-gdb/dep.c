/* dep.c -- changed for emx by Eberhard Mattes -- Jan 1992 */
...
#if defined (EMX)
int
emx_create_inferior (allargs, env)
     char *allargs;
     char **env;
  {
  int pid, argc, a;
  ...
  pid = spawnve (P_DEBUG, nargv[0], (const char * const *)nargv,
...
#if defined (EMX)
extern CORE_ADDR heap_start;
extern CORE_ADDR heap_end;
#endif
     ...
     int from_tty;
{
  ...
  if (filename)
	...
	error ("To look at a core file, you must kill the inferior with \"kill\".");
      corechan = open (filename, O_RDONLY | O_BINARY, 0);
      if (corechan < 0)
...
#if defined (EMX)
        data_end = u.u_data_end;
        data_offset = u.u_data_off;
        heap_start = u.u_heap_base;
        heap_end = u.u_heap_brk;
        heap_offset = u.u_heap_off;
        stack_start = u.u_stack_low;
        stack_end = u.u_stack_end;
        stack_offset = u.u_stack_off;
        reg_offset = (char *)&u.u_regs - (char *)&u;
#else
...
/*	N_SET_MAGIC (core_aouthdr, 0);  */
	...
	{
	  corefile = fconcat (current_directory, filename);
	}
  ...
  if (filename)
    {
      ...
      make_cleanup (free, filename);
#ifdef EMX
      strcpy (exe_fname, filename);
      filename = exe_fname;
      _defext (filename, "exe");
#endif
      
      execchan = openp (getenv ("PATH"), 1, filename, O_RDONLY | O_BINARY, 0,
			&execfile);
      if (execchan < 0)
	perror_with_name (filename);
