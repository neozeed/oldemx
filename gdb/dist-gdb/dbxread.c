/* dbxread.c -- changed for emx by Eberhard Mattes -- Jan 1992 */
     ...
     struct partial_symtab *pst;
{
  ...
  if (pst->ldsymlen || pst->number_of_dependencies)
	...
	perror_with_name (name);
      desc = open(name, O_RDONLY | O_BINARY, 0); /* symbol_file_command
     ...
     char* arg_string;
{
  ...
  *arg_string++ = (char) 0;
  ...
  desc = open (name, O_RDONLY | O_BINARY);
  if (desc < 0)
    perror_with_name (name);
