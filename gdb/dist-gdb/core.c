/* core.c -- changed for emx by Eberhard Mattes -- Jan 1992 */
...
#if defined (EMX)
/* emx has an additional area of memory in the core file. */
...
#if defined (EMX)
/* Offset within core file of start of heap area data.  */
int heap_offset;
#endif
  ...
  if (! have_inferior_p ())
      ...
      if (corefile)
	{
	  printf ("Data segment in core file from 0x%x to 0x%x.\n",
		  data_start, data_end);
#if defined (EMX)
	  printf ("Heap segment in core file from 0x%x to 0x%x.\n",
		  heap_start, heap_end);
#endif
  ...
  while (len > 0)
...
#if defined (EMX)
      /* Note that if there is no core file
         heap_start and heap_end are equal.  */
      else if (memaddr >= heap_start && memaddr < heap_end)
	{
          i = min (len, heap_end - memaddr);
          fileptr = memaddr - heap_start + heap_offset;
	  xferfile = &corefile;
	  xferchan = corechan;
	}
#endif
...
#ifdef EMX
          fileptr = memaddr - exec_data_start + exec_data_offset + exec_off;
#else
...
#ifdef EMX
          fileptr = memaddr - text_start + text_offset + exec_off;
#else
	       ...
	       && memaddr < (corechan >= 0? data_start : exec_data_start))
	{
	  i = min (len, data_start - memaddr);
               ...
               && memaddr >= data_end && memaddr < heap_start)
	{
          i = min (len, heap_start - memaddr);
        }
      else if (corechan >= 0
               && memaddr >= heap_end && memaddr < stack_start)
	{
